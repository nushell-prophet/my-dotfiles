...
def fzf-history-source [] {
    let closure = {
        # Why helper scripts on disk: fzf's reload/preview run nu one-liners via
        # `sh -c`; inlining them means three quoting layers deep. Regenerated on
        # every invocation, so they survive temp cleanup and stay in sync.
        # Why the db path is embedded: `nu -n` resolves $nu.history-path to the
        # plaintext default, not the sqlite file this config sets.
        let src_script = $nu.temp-dir | path join 'fzf-hist-source.nu'
        let preview_script = $nu.temp-dir | path join 'fzf-hist-preview.nu'

        # ROW_NUMBER dedup keeps each command's latest run, newest first
        # (r##' because the content starts with `#`)
        r##'# generated by nushell config.nu (Ctrl+F history picker); safe to delete
def main [--cwd] {
    ...
    | print --no-newline
        ...
        | str replace 'HIST_DB' $'`($nu.history-path)`'
        | save --force $src_script

        r##'# generated by nushell config.nu (Ctrl+F history picker); safe to delete
def main [id: int, command: string] {
    let meta = open HIST_DB
    ...
    $command | nu-highlight
        ...
        | str replace 'HIST_DB' $'`($nu.history-path)`'
        | save --force $preview_script

        let query = commandline
            | str replace --all ' ' '\ '
            | if ($in | is-empty) { '' } else { $'^($in)' }

        let reload = $"reload:nu -n --no-std-lib \"($src_script)\""
        ...
        let selection = '' | ^fzf ...[
            ...
            $'--preview=nu -n --no-std-lib "($preview_script)" {1} {2..}'
        ] | complete
